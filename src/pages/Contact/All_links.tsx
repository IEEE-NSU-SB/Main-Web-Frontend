        ...
        <FadeIn>
            <div className="max-w-[1080px] mx-auto px-4 py-6">
                {loading ? (
                    ...
                    <ErrorMessage message={"Failed to load contact info"} onRetry={refetch} />
                ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                        {/* Address */}
                            ...
                            <div className="flex gap-2 justify-center flex-wrap">
                                {data?.social_media_link.map((link, index) => {
                                    const getSocialIcon = (url: string) => {
                                        ...
                                        if (lowerUrl.includes('facebook')) return <Facebook className="w-4 h-4 text-white" />;
                                        if (lowerUrl.includes('linkedin')) return <Linkedin className="w-4 h-4 text-white" />;
                                        if (lowerUrl.includes('instagram')) return <Instagram className="w-4 h-4 text-white" />;
                                        if (lowerUrl.includes('youtube')) return <Youtube className="w-4 h-4 text-white" />;
                                    ...
                                    return (
                                        <a
                                            ...
                                            className="w-10 h-10 transition-all duration-300 ease-in-out bg-[#002855] rounded flex items-center justify-center hover:bg-[#ffd100]"
                                        >
                                            {getSocialIcon(link)}
                                        </a>
                                    );
