        ...
        <div className="max-w-[1080px] mx-auto px-4 py-10 space-y-10">
          ...
          <div className="flex flex-col md:flex-row justify-between items-center md:items-start gap-6">
            ...
            {data.achievements.length > 0 && (
              <div className="border-2 border-ieee-darkyellow rounded-border-ieee-darkyellow orange border-[2px_2px] rounded-[6px_20px] w-full md:w-80 bg-amber-200 overflow-hidden shadow-[2px_2px_8px_var(--color-ieee-gray-50)]">
                ...
                <ul className="text-sm text-gray-600 space-y-3 max-h-29 overflow-y-auto p-2 bg-white ieee-scrollbar">
                  {data.achievements.map(
                    (ach: { title: string; tenure: string }, i: React.Key) => (
                      <li
                        key={i}
              ...
              {data.roles.map(
                (
                  ...
                  <div key={i}>
                    ...
                    {role.tenure && <p>Tenure: {role.tenure}</p>}

                    <p>
                      {role.current ? "Current Position" : "Position"}:{" "}
                      {role.currentPosition}
                    </p>

                    {role.team && (
                      <p>
                        {role.current ? "Current Team" : "Team"}: {role.team}
                      </p>
                    )}
                  </div>
                )
